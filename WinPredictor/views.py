...
def predict(request) :
    ...
    batting = query=request.GET.get('battingTeam','')
    fielding = query=request.GET.get('fieldingTeam','')
    venue = query=request.GET.get('venue','')
    score = query=request.GET.get('score',0)
    wickets = query=request.GET.get('wickets',0)
    balls = query=request.GET.get('balls',0)
    target = query=request.GET.get('target',0)
    score5 = query=request.GET.get('score5',0)
    wickets5 = query=request.GET.get('wickets5',0)

    batting = d[batting]
    fielding = d[fielding]

    data = [[batting, fielding, int(wickets), int(score), int(target), int(score5), int(wickets5), int(balls), venue]]
